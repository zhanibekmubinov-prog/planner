...
_Обновлено: 2026-09-04, сессия 6 (Cowork). Версия v0.6.1 — понятные ошибки MCP с подсказками (v0.6: проекты, совместный доступ, вкладки-фильтры; v0.5: MCP-коннектор)._
...
| **v0.6.1 Понятные ошибки MCP** | ✅ код в `backend/app/mcp_tools.py` + `docs/MCP_CONNECTOR.md`, `test_mcp.py`/`test_v06.py` ALL OK. **Ждёт коммита и пуша владельцем** (миграций нет) |
...
0. **v0.6.1 — владелец:** `git add . / commit / push` (см. LOG сессии 6). Railway пересоберёт backend, миграций нет.
1. Проверка голосом с телефона: «запиши задачу … в Эмбе в проект <несуществующий>» — Claude должен по `hint` создать проект (`create_project` / `create_project_if_missing`) или уточнить, а не упереться в ошибку. Если снова ошибки — в логах Railway искать `WARNING mcp` (там текст ошибки и аргументы вызова).
