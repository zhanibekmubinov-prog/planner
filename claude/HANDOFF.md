...
_Обновлено: 2026-09-04, сессия 6 (Cowork). Версия v0.7 — чеклист в задаче + перенос задач между проектами перетаскиванием; v0.6.1 — понятные ошибки MCP (v0.6: проекты, совместный доступ; v0.5: MCP-коннектор)._
...
| **v0.6.1 Понятные ошибки MCP** | ✅ код в `backend/app/mcp_tools.py` + `docs/MCP_CONNECTOR.md`, `test_mcp.py`/`test_v06.py` ALL OK. **Ждёт коммита и пуша владельцем** |
| **v0.7 Чеклист + перенос задач между проектами** | ✅ код: `Task.checklist` (миграция `b3c4d5e6f7a8`), `Checklist.tsx`, `TaskPanel`/`Board`/`DirectionPage`, MCP `add_checklist_items`/`check_item`. `npm run build` чисто, скриншоты на стенде. **Ждёт `alembic upgrade head`, `npm run build`, коммита и пуша** |
...
0. **v0.6.1 + v0.7 — владелец:** в `backend` (venv) `alembic upgrade head` → `alembic current` = `b3c4d5e6f7a8 (head)`; в `frontend` `npm run build`; `git add . / commit / push`. В логах Railway backend — `Running upgrade a2b3c4d5e6f7 -> b3c4d5e6f7a8`. На сайте в углу — v0.7.
1. Проверка в проде: открыть направление → перетащить задачу с карточки проекта на другой проект / «Без проекта»; в карточке задачи добавить пункты чеклиста (Enter), отметить — на карточке доски появится `2/5`. Голосом: «добавь в задачу ГРП пункты: …», «отметь пункт …».
1a. Проверка голосом с телефона: «запиши задачу … в Эмбе в проект <несуществующий>» — Claude должен по `hint` создать проект (`create_project` / `create_project_if_missing`) или уточнить, а не упереться в ошибку. Если снова ошибки — в логах Railway искать `WARNING mcp` (там текст ошибки и аргументы вызова).
2. Проверка v0.6 в проде с коллегой: проект в направлении, перенос задач в проект, «Поделиться…» → коллега видит в «Общие» и в сайдбаре с ⇄.
3. Дальше по желанию: роли «руководитель/команда» поверх шаринга; список задач проекта в сайдбаре; кнопка «Подключить Claude» в профиле; инструменты MCP для майндмапов; агенты тулов (шаг 6).
