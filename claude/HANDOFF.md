...
| **v1.2 Имена кириллицей ↔ латиницей (MCP)** | ✅ `app/names.py` (фонетический ключ, `person_matches`), `mcp_tools.py` (`_match(..., fuzzy=)`, `resolve_person`, `_similar_people`, `create_person`, подсказки и инструкции), версия 1.2 в `main.py`/`Sidebar.tsx`; `tests/test_v12_names.py` 49 тестов, pytest **258 passed**, `test_mcp.py`/`test_v06.py` ALL OK. Без миграции. **Ждёт: коммит/пуш, проверить голосом в проде** |
...
0. **Владелец — v1.2 (имена):** коммит/пуш. После деплоя сказать Claude «поручи <задача> Айдосу» для человека, записанного латиницей — должен найти без дубля; при двух похожих спросит, кого имели в виду.
