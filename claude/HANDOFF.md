...
_Обновлено: 2026-09-07, сессия 9 (Cowork). **Версия v0.9** — бот планнера сам показывает chat id и привязывает Telegram по кнопке «Подключить Telegram» (миграция не нужна; ждёт переменных в Railway, `setWebhook` и коммита). Сессия 8 — презентация для коллег (`claude/PRESENTATION.md`). v0.8 — починка по итогам адверсариального ревью: корзина, архив, «без направления», перенос проекта, права, OAuth, планировщик, безопасный confirm/автосейв. v0.7 — чеклист + перенос задач перетаскиванием; v0.6.1 — понятные ошибки MCP (v0.6: проекты и совместный доступ; v0.5: MCP-коннектор)._
