...
- Сайдбар (v0.4): разделы наверху одним блоком (Карта, Мне поручено, Все задачи, Майндмапы, Люди, Тулы), направления — раскрывающимся списком с фильтром; на направлении работает правая кнопка мыши (мини-меню: открыть, майндмапы, переименовать, изменить, пауза, архив, удалить). Страницы списков — колонка по центру экрана (≤1080px), не на всю ширину.
...
- Код: `backend/app/routers/mcp_oauth.py`, `routers/mcp.py`, `mcp_tools.py`; документация `docs/MCP_CONNECTOR.md`.
