...
import { api, Direction, Person, Task, Tool } from "./api";

export type Store = {
  directions: Direction[]; tasks: Task[]; people: Person[]; tools: Tool[];
...
export function useStore(): Store {
  ...
  const reload = useCallback(async () => {
    setLoading(true);
    await Promise.all([reloadDirections(), reloadTasks(), reloadPeople(), reloadTools()]);
    setLoading(false);
  }, [reloadDirections, reloadTasks, reloadPeople, reloadTools]);

  useEffect(() => { void reload(); }, [reload]);

  const patchTask = useCallback((t: Task) => setTasks((prev) => prev.map((x) => (x.id === t.id ? t : x))), []);

  return { directions, tasks, people, tools, loading, error, reload, reloadTasks, reloadDirections, reloadPeople, reloadTools, patchTask, setError };
}
